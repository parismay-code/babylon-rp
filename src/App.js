...
const App = () => {
  ...
  React.useEffect(() => {
    window.alt.on("cef::player:setData", (obj) =>
      playerStore.fetchPlayerState(obj)
    );
  }, [playerStore]);
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    window.alt.on("cef::bank:setWithdrawHistory", (array) =>
      bankStore.fetchWithdrawHistory(array)
    );
    window.alt.on("cef::bank:addWithdraw", (data) =>
      bankStore.addWithdraw(data)
    );
    window.alt.on("cef::bank:setTopUpHistory", (array) =>
      bankStore.fetchTopUpHistory(array)
    );
  ...
  React.useEffect(() => {
    ...
    window.alt.on("cef::hud:loadCarState", (obj) =>
      hudStore.fetchCarState(obj)
    );
  }, [hudStore]);
  React.useEffect(() => {
    window.alt.on("cef::chat:setAccess", (obj) => chatStore.fetchAccess(obj));
    window.alt.on("cef::chat:loadFamilyIcon", (icon) =>
      chatStore.fetchFamilyIcon(icon)
    );
  }, [chatStore]);
  React.useEffect(() => {
    window.alt.on("cef::creator:setData", (data) =>
      creatorStore.fetchData(data)
    );
    window.alt.on("cef::creator:setClothes", (data) =>
      creatorStore.fetchClothes(data)
    );
  }, [creatorStore]);
  React.useEffect(() => {
    window.alt.on("cef::battlePass:setQuests", (data) =>
      battlePassStore.fetchQuests(data)
    );
    window.alt.on("cef::battlePass:setAwards", (data) =>
      battlePassStore.fetchAwards(data)
    );
    window.alt.on("cef::battlePass:setSeason", (name) =>
      battlePassStore.setSeason(name)
    );
    window.alt.on("cef::battlePass:setCoins", (value) =>
      battlePassStore.setSeason(value)
    );
  }, [battlePassStore]);
  React.useEffect(() => {
    window.alt.on("cef::shop:setOwner", (name) => shopsStore.setOwner(name));
    window.alt.on("cef::clothesShop:setData", (array) =>
      shopsStore.fetchClothesShopData(array)
    );
  }, [shopsStore]);
  ...
  return (
    <div className="app">
      {component === "auth" && (
        <Auth isRegistered={isRegistered} store={authStore} />
      )}
      ...
      {component === "bank" && (
        <Bank store={bankStore} player={playerStore.playerState} />
      )}
      {component === "hud" && (
        <HUD
      ...
      {component === "adminRedactor" && (
        <AdminRedactor data={adminRedactorData} />
      )}
      ...
      {component === "pay" && (
        <Pay player={playerStore.playerState} payPrice={payPrice} />
      )}
      ...
      <Chat
        ...
        isVisible={
          (component === "hud" || component === "clothesShop") &&
          currentHUD === 0 &&
          !playerStore.playerState.dead.isDead
        }
      />
    </div>
  );
