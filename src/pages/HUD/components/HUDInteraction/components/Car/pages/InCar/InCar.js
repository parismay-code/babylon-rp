...
const InCar = ({
	...
	const arendHouseIcon = React.useMemo(() => {
			return <svg xmlns="http://www.w3.org/2000/svg" width="44.727" height="30.103" viewBox="0 0 44.727 30.103"
			            fill={currentOption === 'arendHouse' || nextOption === 'arendHouse' ? '#eaeaea' : null}
			            opacity={currentOption === 'arendHouse' || nextOption === 'arendHouse' ? '1' : '.5'}
			            style={currentOption === 'arendHouse' || nextOption === 'arendHouse' ? {filter: 'drop-shadow(0 2px 2px #00000080)'} : null}>
		...
		sellBusinessIcon = React.useMemo(() => {
			return <svg xmlns="http://www.w3.org/2000/svg" width="36.25" height="29" viewBox="0 0 36.25 29"
			            fill={currentOption === 'sellBusiness' || nextOption === 'sellBusiness' ? '#eaeaea' : null}
			            opacity={currentOption === 'sellBusiness' || nextOption === 'sellBusiness' ? '1' : '.5'}
			            style={currentOption === 'sellBusiness' || nextOption === 'sellBusiness' ? {filter: 'drop-shadow(0 2px 2px #00000080)'} : null}>
				<path
		...
		sellHouseIcon = React.useMemo(() => {
			return <svg xmlns="http://www.w3.org/2000/svg" width="34.606" height="32.296" viewBox="0 0 34.606 32.296"
			            fill={currentOption === 'sellHouse' || nextOption === 'sellHouse' ? '#eaeaea' : null}
			            opacity={currentOption === 'sellHouse' || nextOption === 'sellHouse' ? '1' : '.5'}
			            style={currentOption === 'sellHouse' || nextOption === 'sellHouse' ? {filter: 'drop-shadow(0 2px 2px #00000080)'} : null}>
				<g transform="translate(-943.706 -143.021)">
					<g transform="translate(12462.205 -8898.496)">
						<path
							d="M20.856,28.967H4.526a.906.906,0,0,1-.905-.9V17.6a1.313,1.313,0,0,1,.126-.073.874.874,0,0,0,.184-.117L16.294,6.5,28.67,17.42a.828.828,0,0,0,.174.11,1.221,1.221,0,0,1,.123.071v.527a7.5,7.5,0,0,0-8.111,10.838ZM2.117,16.294a.907.907,0,0,1-.674-.3L.232,14.647A.907.907,0,0,1,.3,13.368L14.783.581A2.244,2.244,0,0,1,16.29,0a2.274,2.274,0,0,1,1.518.583L23.536,5.64V2.715a.906.906,0,0,1,.9-.9h3.622a.906.906,0,0,1,.9.9v7.72l3.321,2.932a.905.905,0,0,1,.068,1.278l-1.21,1.347a.907.907,0,0,1-1.279.069L16.893,4.618a.906.906,0,0,0-1.2,0L2.722,16.062A.9.9,0,0,1,2.117,16.294Z"
		...
		sellCarIcon = React.useMemo(() => {
			return <svg xmlns="http://www.w3.org/2000/svg" width="34.453" height="25.203" viewBox="0 0 34.453 25.203"
			            fill={currentOption === 'sellCar' || nextOption === 'sellCar' ? '#eaeaea' : null}
			            opacity={currentOption === 'sellCar' || nextOption === 'sellCar' ? '1' : '.5'}
			            style={currentOption === 'sellCar' || nextOption === 'sellCar' ? {filter: 'drop-shadow(0 2px 2px #00000080)'} : null}>
		...
		arendCarIcon = React.useMemo(() => {
			return <svg xmlns="http://www.w3.org/2000/svg" width="44.414" height="24.111" viewBox="0 0 44.414 24.111"
			            fill={currentOption === 'arendCar' || nextOption === 'arendCar' ? '#eaeaea' : null}
			            opacity={currentOption === 'arendCar' || nextOption === 'arendCar' ? '1' : '.5'}
			            style={currentOption === 'arendCar' || nextOption === 'arendCar' ? {filter: 'drop-shadow(0 2px 2px #00000080)'} : null}>
	...
	return <div ref={screen} className="hud-interactions-car-in">
		<div className="hud-interactions-car-in-navigation">
			<div
				className="hud-interactions-car-in-navigation-back"
				onClick={() => setCurrentPage('main')}
			>
				<img className="hud-interactions-car-in-navigation-back__arrow" src={rightArrow} alt="#"/>
				<span className="hud-interactions-car-in-navigation-back__title">назад</span>
			</div>
		</div>
		<div className="hud-interactions-car-in__title">{titles[nextOption]}</div>
		<div className="hud-interactions-car-in-options">
			<div
				className="hud-interactions-car-in-options__element hud-interactions-car-in-options__element_arendHouse"
			...
			<div
				className="hud-interactions-car-in-options__element hud-interactions-car-in-options__element_sellBusiness"
			...
			<div
				className="hud-interactions-car-in-options__element hud-interactions-car-in-options__element_sellHouse"
			...
			<div
				className="hud-interactions-car-in-options__element hud-interactions-car-in-options__element_sellCar"
			...
			<div
				className="hud-interactions-car-in-options__element hud-interactions-car-in-options__element_arendCar"
