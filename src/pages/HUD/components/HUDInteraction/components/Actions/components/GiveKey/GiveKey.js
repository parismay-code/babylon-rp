...
const GiveKey = ({currentOption, prevOption, player, transport}) => {
    ...
    const toggleStyles = React.useMemo(() => {
        ...
        else if (currentOption === 'giveCarKey' && prevOption)
            return {
                visibility: 'visible',
                width: '100%',
                height: '100%',
    ...
    return <div
        className='hud-interactions-actions-give-car-key'
        style={toggleStyles}
    >
        <div className='hud-interactions-actions-give-car-key-header'>
            <div className='hud-interactions-actions-give-car-key-header-nickname'>
                <span
                    className='hud-interactions-actions-give-car-key-header-nickname__firstname'>{player?.nickname?.firstname}</span>
                <span
                    className='hud-interactions-actions-give-car-key-header-nickname__lastname'>{player?.nickname?.lastname}</span>
                <div className='hud-interactions-actions-give-car-key-header-nickname__id'>ID: {player?.id}</div>
            </div>
        </div>
        <div className='hud-interactions-actions-give-car-key-content'>
            <div className='hud-interactions-actions-give-car-key-content-choose-transport'>
                <div className='hud-interactions-actions-give-car-key-content-choose-transport-header'>Выберите транспорт
                </div>
                <div className='hud-interactions-actions-give-car-key-content-choose-transport-list'
                     style={motorcycles.length > 0 ? {display: 'flex'} : {display: 'none'}}>
                    {motorcycles.map((el, key) => {
                        return <svg
                            className='hud-interactions-actions-give-car-key-content-choose-transport-list__element'
            ...
            <div className='hud-interactions-actions-give-car-key-content-info'>
                ...
                <div className='hud-interactions-actions-give-car-key-content-info-cost'>
                    ...
                    <div className='hud-interactions-actions-give-car-key-content-info-cost__value'>{cost}</div>
                </div>
            </div>
        </div>
    </div>
