...
const HUDMic = ({store, player}) => {
	...
	React.useEffect(() => {
		EventManager.addHandler('hud', 'toggleMicrophone', (bool) => toggleMicrophone(bool));
		EventManager.addHandler('hud', 'toggleRadioSet', (bool) => toggleRadioSet(bool));
		EventManager.addHandler('hud', 'setNewSpeaker', (obj) => store.fetchSpeaker(obj));
		EventManager.addHandler('hud', 'removeSpeaker', (id) => store.removeSpeaker(id));
	}, [store]);
