...
const ClothesShop = ({player, hudStore, store}) => {
	...
	const emitPreview = React.useCallback((component, drawable, texture) => EventManager.emitClient('clothesShop', 'preview', {
		component,
		drawable,
		texture,
	}), []);
	...
	const emitBuy = React.useCallback(() => {
		...
		EventManager.emitServer('clothesShop', 'buy', obj, price);
	...
	React.useEffect(() => {
		EventManager.addHandler('clothesShop', 'setData', array => store.fetchClothesShopData(array));
		
		EventManager.stopAddingHandlers('clothesShop');
		
		return () => EventManager.removeTargetHandlers('clothesShop');
	}, [store]);
	
	return <div ref={screen} className="clothes-shop">
		<div className="clothes-shop-content">
			<ClothesList store={store} currentType={currentType} currentElement={currentElement}
			             setElement={setElement}/>
			<div className="clothes-shop-content-bottom">
				<ClothesType currentType={currentType} setType={setType} handleChangeTexture={handleChangeTexture}
